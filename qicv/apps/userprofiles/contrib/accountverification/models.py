...
class AccountVerificationManager(models.Manager):
    def authorize_user(self, user):
        """Give an active user authorization to see profile listings"""
        verification = self.get(user=user)
        if verification.is_active():
            # if the user has activated their email address, authorize them
            verification.activation_key = self.model.AUTHORIZED
            verification.save()
            return user
        else:
            # user is not active
...
class AccountVerification(models.Model):
    ...
    def is_active(self):
        return self.activation_key in [self.ACTIVATED, self.AUTHORIZED]

    def is_authorized(self):
        return self.activation_key == self.AUTHORIZED
